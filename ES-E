...
# 2. 資料庫輔助與轉檔函式
# ==========================================
def get_scraped_tags(db_name='RDKS-E.db'):
    """讀取資料庫，獲取所有已經抓取過的 carTag 集合 (作為防重複護城河)"""
...
def auto_export_sql(db_name='RDKS-E.db', sql_name='RDKS-E_Backup.sql'):
...
def save_batch_to_sql(batch_data, db_name='RDKS-E.db'):
    """獨立的批次存檔函式"""
...
def main_scraper_all():
    # 使用安全的資料夾名稱
    folder_name = sanitize_filename("RDKS-E")
    ...
    # --- 建立帶有「防重複護城河」的資料表 ---
    conn = sqlite3.connect('RDKS-E.db')
    cursor = conn.cursor()
    cursor.execute('''
    ...
    time_is_up = False # 紀錄是否已經超時

    for target_brand in brands:
        if time_is_up: break # 如果超時，跳出最外層品牌迴圈
            ...
            for tg_data in type_groups:
                ...
                for version in versions:
                    ...
                    if time.time() - PROGRAM_START_TIME > MAX_RUNTIME_SECONDS:
                        print("\n⚠️ [超時警告] 執行時間已達 5.5 小時極限！準備進入安全暫停程序...")
                        time_is_up = True
                        break 
                    ...
                    if car_tag in scraped_tags:
                        ...
                        # 🌟 智能更新邏輯 2：如果「連續 5 筆」都是舊資料，推測該車系無新車，直接跳過後續舊車！
                        if already_exists_streak >= 5:
                            print(f"    ⏭️ [跳過舊車] 型號 {tg_id} 連續 5 筆無更新，跳過剩餘版本。")
                            break # 跳出 versions 迴圈，檢查下一個型號
                        continue 
                    else:
                        # 如果發現新車，就把連續紀錄歸零
                        already_exists_streak = 0
        ...
        # 品牌迴圈結束 (或因超時中斷) 時，確保存下剩餘沒滿 100 筆的資料
        if batch_data:
            save_batch_to_sql(batch_data)
            batch_data = []
        ...
        try:
            ...
            if not df_brand.empty:
                ...
                print(f"  📊 【{target_brand}】Excel 匯出成功！庫內共 {len(df_brand)} 筆。")
        except Exception as e:
